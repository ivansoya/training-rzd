...
CATALOGUE = [
    ...
    {
        "op": "RandomAreaCrop", "group": "geometry",
        "name": "Случайный кусок кадра",
        "why": "Берёт случайный кусок заданной доли и позволяет объектам "
               "уходить за край — так они и появляются в кадре на самом деле. "
               "Объект, от которого видно меньше порога, из разметки "
               "выбрасывается: учить по четверти предмета нечему.",
        "params": {
            "scale": {
                "names": ["scale"], "label": "Доля стороны",
                "hint": "0,6 — это кусок в 60 % ширины и высоты кадра.",
                **_rng(0.2, 1.0, [0.6, 0.9], step=0.05),
            },
            "min_visibility": {
                "names": ["min_visibility"], "label": "Порог видимости",
                "hint": "Ниже этой доли объект из разметки выбрасывается.",
                **_num(0.1, 1.0, 0.5, step=0.05),
            },
        },
    },
    {
        "op": "FractionCrop", "group": "geometry",
        "name": "Кроп",
        "why": "Вырезает заданное окно кадра — границы долями ширины и высоты. "
               "Объект, от которого в окне осталось меньше порога, из "
               "разметки выбрасывается.",
        "params": {
            "x_range": {
                "names": ["x_range"], "label": "По ширине, от и до",
                **_rng(0.0, 1.0, [0.0, 1.0], step=0.01),
            },
            "y_range": {
                "names": ["y_range"], "label": "По высоте, от и до",
                **_rng(0.0, 1.0, [0.0, 1.0], step=0.01),
            },
            "min_visibility": {
                "names": ["min_visibility"], "label": "Порог видимости",
                **_num(0.0, 1.0, 0.5, step=0.05),
            },
        },
    },
    {
