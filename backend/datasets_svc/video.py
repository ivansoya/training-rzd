...
def count_frames(path):
    """Сколько всего кадров. Нужно размечаемому видео: таймлайн ходит по
    номерам, и последний кадр должен быть известен заранее.

    Контейнер иногда знает число кадров сам; если нет — считаем из частоты и
    длительности. Полный проход по декодеру ради точного числа не делаем:
    на часовом ролике это минуты ожидания при загрузке.
    """
    try:
        import av
    except ImportError as exc:  # noqa: BLE001
    ...
    try:
        with av.open(path) as container:
            ...
            if stream.duration and stream.time_base and stream.average_rate:
                ...
                return int(seconds * float(stream.average_rate))
    except Exception:  # noqa: BLE001
        return None
    return None


def grab_frame(path, frame_no):
    """Кадр по номеру — ровно тот, что уйдёт в разметку.

    Перемотка идёт на ближайший опорный кадр перед целью, дальше декодер
    крутится вперёд. Иначе на длинном ролике каждый переход стоил бы полного
    прохода с начала, а без перемотки вообще — десятки секунд на кадр.
    """
    try:
        import av
    except ImportError as exc:  # noqa: BLE001
    ...
    with av.open(path) as container:
        ...
        target_seconds = frame_no / rate
        # Небольшой запас назад: опорный кадр ищется по времени, и попадание
        # ровно в цель означало бы риск перемотаться уже за неё.
        offset = max(0, int((target_seconds - 0.5) / time_base))
        try:
        ...
        if last is not None:
            # Просили кадр за концом ролика — отдаём последний, а не ошибку:
            # на границе таймлайна это ожидаемое поведение, а не сбой.
            return last.to_image()
    raise VideoError("Кадр не найден.")
