...
Заслонённость хранится отрезками у трека, но **края отрезка обязаны совпадать
с ключами**: зону задают ключом исчезновения, и она живёт от него до следующего
ключа. Отсюда плата, о которой стоит знать заранее: перетаскивая граничный
ключ, разметчик одним жестом двигает и положение объекта, и край зоны. Прежде
здесь было ровно обратное решение — края были сами по себе, — и отказались от
него сознательно: невидимая граница, не совпадающая ни с одним ключом, не имеет
ручки, за которую её можно поймать.
"""
...
def move_boundary(ranges, frm: int, to: int) -> list[list[int]]:
    """Ключ переехал — край зоны, который он держал, едет за ним.

    Отрезок, у которого края сошлись или перевернулись, исчезает: нулевой
    невидимости не бывает, а `normalize_ranges` такие пары и так выбрасывает.
    """
    moved = [
        [to if start == frm else start, to if end == frm else end]
        for start, end in (ranges or [])
    ]
    return normalize_ranges(moved)


def drop_boundary(ranges, frame: int) -> list[list[int]]:
    """Ключа не стало — зона, которую он держал краем, исчезает вместе с ним.

    Зону держат оба края, поэтому проверяем и начало, и конец: снятый ключ в
    середине отрезка не значит ничего, снятый на краю — обрывает зону.
    """
...
def empty_frames(tracks: list[dict], singles: list[dict], marks) -> list[int]:
    """Помеченные кадры, на которых и правда никого нет.

    Проверяем **присутствие объекта**, а не попадание в план выгрузки. Это не
    одно и то же: у трека бывает шаг выгрузки, и кадр между выгружаемыми в план
    не попадает, хотя объект на нём есть. Назвать такой кадр фоновым значило бы
    соврать в датасете — модель училась бы, что здесь пусто, глядя на объект.

    Пометка на занятом кадре не выбрасывается, а просто не действует: объект
    уберут — кадр снова станет фоновым.
    """
    busy = {int(s["frame_no"]) for s in singles}
    prepared = [
        (track, sorted(track.get("keys") or [], key=lambda k: k["frame_no"]))
        for track in tracks
    ]
    out = []
    for frame_no in sorted({int(f) for f in marks or []}):
        if frame_no in busy:
            continue
        if any(box_at(track, keys, frame_no) for track, keys in prepared):
            continue
        out.append(frame_no)
    return out
